<script setup>
import DetachedActions from './DetachedActions.vue'
</script>
<template>
  <div
    ...
    <div class="flex items-center flex-1">
      ...
      <div class="h-9 ml-auto flex items-center pr-2 md:pr-3">
        ...
        <DetachedActions
          ...
          :actions="availableDetachedActions"
        ...
        <div class="hidden md:flex px-2">
          <ActionSelector
            v-if="shouldShowActionSelector && availableNovaActions.length"
            :resource-name="resourceName"
            :via-resource="actionQueryString.viaResource"
            :via-resource-id="actionQueryString.viaResourceId"
            :via-relationship="actionQueryString.viaRelationship"
            :actions="availableNovaActions"
            :pivot-actions="pivotActions"
            :pivot-name="pivotName"
            :endpoint="actionsEndpoint"
            :selected-resources="selectedResourcesForActionSelector"
...
export default {
  ...
  computed: {
    ...
    availableDetachedActions() {
      if (this.availableActions) {
        return this.availableActions.filter((item) => item.showDetached)
      }
      return {}
    },
    availableNovaActions() {
      if (this.availableActions) {
        return this.availableActions.filter((item) => !item.showDetached)
      }
      return {}
